...
with open(DATA_FILE, mode="w", newline="") as file:
    ...
    writer.writerow(["Tiempo", "Precio_Banesco", "Precio_Bank_Transfer", "Precio_Provincial", "Precio_Mercantil"])

# Función para guardar datos en un archivo CSV
def guardar_datos_csv(tiempo, precio_banesco, precio_bank_transfer, precio_provincial, precio_mercantil):
...
def reiniciar_datos_diarios():
    ...
    """
    global last_reset_date, tiempos, precios_banesco, precios_bank_transfer, precios_provincial, precios_mercantil
...
def actualizar_datos():
    ...
    precio_provincial = loop.run_until_complete(obtener_tasa_usdt_ves(["Provincial"]))
    precio_mercantil = loop.run_until_complete(obtener_tasa_usdt_ves(["Mercantil"]))
    ...
    precios_provincial.append(
        precio_provincial if precio_provincial is not None
        else (precios_provincial[-1] if precios_provincial else 0)
    )
    precios_mercantil.append(
        precio_mercantil if precio_mercantil is not None
        else (precios_mercantil[-1] if precios_mercantil else 0)
    )

    # Guardamos los datos en el CSV
    guardar_datos_csv(tiempo, precios_banesco[-1], precios_bank_transfer[-1], 
                     precios_provincial[-1], precios_mercantil[-1])
